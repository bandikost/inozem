  ...
  return (
    <section className="flex flex-col items-center">
      <div className="flex gap-4 items-center mt-12">
        ...
        <button onClick={() => setAscending(prev => !prev)} className="px-4 py-2 bg-purple-600 text-white rounded">
          Сортировка: {ascending ? 'А → Я' : 'Я → А'}
        </button>
      </div>
    <div className='flex items-center mt-8'> 
        <input value={value} onChange={(e:any) => setValue(e.target.value)} className='border ' />
        <button className="px-4 py-0.5 ml-2 bg-purple-600 text-white rounded cursor-pointer hover:bg-purple-900">Найти</button>
    </div>
