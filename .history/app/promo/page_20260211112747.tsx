...
export default async function Page() {
    ...
    <section className='flex flex-col justify-center pb-20'>
        ...
        <div className="grid grid-cols-2 gap-8 mt-8">
            {promo.map(promotion => (
            <div key={promotion.id} className="border-2 border-dotted border-zinc-300 rounded shadow-2xl bg-white p-6 relative">
                ...
                <small className="text-zinc-500 text-sm mt-8 absolute bottom-5 left-5">{new Date(promotion.created_at).toLocaleDateString('ru-RU')}</small>
            </div>
            ))}
        </div>
    </section>
