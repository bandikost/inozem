...
export async function POST(req: NextRequest) {
     try {
        const { last_name, patronymic, name, phone, email, education_level, specialization, created_at} = await req.json()
    ...
    const [result] = await db.query<ResultSetHeader>(
      ...
      [name, last_name, patronymic, email, phone, education_level, specialization, created_at]
        ...
        await sendApplicationEmail(name, last_name, patronymic, email, phone, education_level, specialization, created_at)
