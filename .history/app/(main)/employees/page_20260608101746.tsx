...
<section className="flex flex-col px-4 mt-30">
    <h1 className="text-prpl font-semibold text-center">Преподаватели ЧОУ ДПО «Академия медицинского образования им.Ф.И.Иноземцева»</h1>
        ...
        <div className="grid gap-3 grid-cols-2 mb-15 mt-5">
            ...
            .slice(0, 10)
            .map(teacher => (
                <div key={teacher.id} className="flex list-none border border-gray-300  mt-8 rounded shadow-2xl bg-white">
                    {teacher.photo_url ? (
                        <ImageWithSkeleton
                            src={teacher.photo_url}
                            alt={`Сотрудник ${teacher.last_name}`}
                            wrapperClassName="mr-3 h-[300px] w-[250px] rounded object-cover"
                            aspect="1/1"
                        />
                        ) : (
