...
export default function ActivityEditor({
  ...
  const [form, setForm] = useState<FormData>({
    ...
    attendance_control: activity.attendance_control ?? "",
    location: activity.location ?? "",
    planned_results: activity.planned_results ?? "",
  });
    ...
    <section className="mx-auto mt-27 w-full px-6 pb-20">
      ...
      >
        ...
        >
          ...
          <Field label="Контроль присутствия">
              <textarea
              name="attendance_control"
              value={form.attendance_control}
              onChange={handleChange}
              rows={5}
              placeholder="Опишите порядок контроля присутствия..."
              className="input-main resize-y"
              />
              </Field>
              ...
              onChange={handleChange}
              rows={5}
              placeholder="Адрес или описание места проведения..."
              className="input-main resize-y"
              />
              </Field>


              <Field label="Планируемые результаты">
              <textarea
              name="planned_results"
              value={form.planned_results}
              onChange={handleChange}
              rows={5}
              placeholder="Какие результаты получит участник..."
              className="input-main resize-y"
              />
              </Field>
