...
export default function LinksEditor({
  ...
  useEffect(() => {
  const saved = localStorage.getItem("links-collapsed");
  if (saved) setCollapsed(JSON.parse(saved))
}, [])

useEffect(() => {
  localStorage.setItem("links-collapsed", JSON.stringify(collapsed))
}, [collapsed])
    ...
    <div className="p-4">
      ...
      <div className="flex flex-col gap-6">
        ...
        {links.map((group, groupIndex) => (
          <div key={group.id} className="border border-gray-400 rounded-md p-4">

            <div className="flex justify-between items-center mb-3">
              <button onClick={() => toggleGroup(group.id)} className="button-more">{collapsed[groupIndex] ? "Развернуть" : "Свернуть"}</button>
            </div>
