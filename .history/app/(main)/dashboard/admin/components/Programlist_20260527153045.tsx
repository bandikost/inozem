...
export default function ProgramList({program} : Props) {
    ...
    return (
        ...
        <div className="flex flex-col items-center border border-gray-300 w-full rounded-md bg-green pt-5 pb-8 mt-4">
         <div className="flex flex-col items-start px-4">
                <h1 className="text-white !font-normal mt-10">Каталог программ</h1>
                <h3 className="text-white !font-normal mt-2">Вы находитесь на странице редактирования контентном программ</h3>
            ...
            <ul className="grid gap-4 mt-8">
                {filtred.map(p =>
                    <div key={p.id} className="grid items-center text-center grid-cols-3 gap-6 border border-gray-300 shadow-md rounded-md p-4 !text-lg">
                        <p className="!text-xl opacity-70">{p.name}</p>
                        <p className="!text-default !font-normal opacity-70">{p.time} академических{" "}{getHourWord(p.time)}</p>
