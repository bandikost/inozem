...
export default async function Page({ params }: ProgramsPageProps) {
  ...
  const baseBlock = title.find((item) => program.category === "Профессиональная переподготовка" && program.education === "Среднее" && <BaseVideo />)


 {
    blocks.push({
      title: currentTitle?.blocks[0] || "",
      component: baseBlock || 0,
    })
  }

   {
    blocks.push({
      ...
      component: <Main specialization={specialization} category={category} />,
    })
  }

  blocks.push({
    title: currentTitle?.blocks[2] || "",
    component:  <Second specialization={specialization} category={category} />,
  })

  blocks.push({
    title: currentTitle?.blocks[3] || "",
    component: <Three specialization={specialization} category={category} />,
  })
  ...
  blocks.push({
    ...
    component: (
      <div>
        <h3>Файлы для каждой специальности</h3>
      </div>
    ),
  })
