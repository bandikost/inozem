...
export default function ProgramSelect({ program, userId, initialTime }: ProgramSelectProps) {
 ...
 const options = useMemo(() => {
  ...
  if (program.time !== undefined && program.time !== null) {
    arr.push({
      time: program.time,
      price: program.price,
    });
  ...
  if (program.time_secondary !== undefined && program.time_secondary !== null) {
    arr.push({
  ...
  const defaultTime =
  initialTime && options.some(o => o.time === Number(initialTime))
    ? Number(initialTime)
    : options[0]?.time ?? null
  ...
  const [selectedTime, setSelectedTime] = useState<number | null>(defaultTime)
    ...
    <div className="mt-12 flex flex-col sm:flex-row items-center justify-between gap-6">
      ...
      {options.length > 1 ? (
        <label className="flex items-center gap-2">
          ...
          <select value={selectedTime ?? ""} onChange={handleTimeChange} className="border border-gray-400 p-1 rounded">
            {options.map((o) => (
