      ...
      <div className="border border-gray-300 rounded-md shadow-2xl p-6 flex flex-col gap-8">
        ...
        {hasLinks && links.map((group, i) => (
          ...
          <div key={i}>
            ...
            {group.title?.trim() && (
  <h2 className="text-prpl !text-2xl mb-4">
    {group.title}
  </h2>
)}   
            <ul className="flex flex-col gap-2">
              ...
              {group.items.map((link) => (
                ...
                <li key={link.href}>
                  <a  href={link.href}  target="_blank" className="hover:underline text-lg">{link.name}</a>
                </li>
        ...
        {hasVideos && (
          <MediaGallery
            cols="lg:grid-cols-3 grid-cols-1 xs:grid-cols-2 md:grid-cols-2"
            items={sources}
          />
        )}
