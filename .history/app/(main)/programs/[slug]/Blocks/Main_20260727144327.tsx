                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {group.items.map((link) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="group flex items-center justify-between flex-col xs:flex-row rounded-2xl border border-slate-200 bg-white px-4 py-4 text-base text-slate-800 shadow-sm transition-all hover:-translate-y-0.5 hover:border-violet-300 hover:shadow-md md:text-lg"
                      >
                        <span className="pr-4 leading-snug">{link.name}</span>
                        <span className="shrink-0 rounded-full bg-violet-50 px-3 py-1 text-xs font-medium text-violet-700 transition-colors group-hover:bg-violet-100">
                          открыть
                        </span>
                      </a>
                    ))}
