import { getProgramBySlug, getProgramStructure, hasUserProgram } from "@/lib/programm";
...
export default async function Page({ params }: ProgramsPageProps) {
  ...
  const structure = await getProgramStructure(program.id)
  ...
  (structure ?? []).map((block: any) => {
    switch (block.type) {
      ...
      case 'third':
        return {
          title: block.title,
          component: <Three category={category} specialization={specialization} />
        }
