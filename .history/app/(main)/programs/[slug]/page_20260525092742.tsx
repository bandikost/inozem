...
const blocks: ProgramSliderBlock[] =
  currentBlocks
    .map((block: any): ProgramSliderBlock | null => {
      ...
      switch (block.type) {
        ...
        case "four":
          return {
            title: block.title,
            component: (
              <Four sources={block.data?.sources ?? []}/>
            ),
          };
    ...
    .filter(
      (b: any): b is ProgramSliderBlock => b !== null
    )
console.log("BLOCKS FROM DB:", currentBlocks)
  ...
  return (
    <section className="prose mx-auto px-6 mt-27 mb-10">
      ...
      {!hasAccess ? ( 
        ...
        <pre>{JSON.stringify(currentBlocks, null, 2)}</pre>
        </>
