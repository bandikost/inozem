    ...
    <div className="mt-10 flex flex-col gap-6">
      ...
      <div className="h-[100px] flex items-center gap-2 overflow-x-auto">
        {blocks.map((block, index) => (
          <button key={index} onClick={() => setCurrent(index)} className={`px-3 py-2 text-xl text-[#202027] rounded-md border whitespace-nowrap ${ current === index ? 'button-more' : 'button-more-bulge opacity-20' }`}>
            {block.title}
          </button>
        ))}
      </div>
