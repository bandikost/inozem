...
export default function ProgramSlider({ blocks, themes }: ProgramSliderProps) {
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mt-4 flex-wrap px-6 items-center justify-center">
        {blocks.map((_, index) => (
          <button key={index} onClick={() => setCurrent(index)} className={`w-10 h-10 rounded-md border ${current === index ? 'button-more' : 'button-more-bulge'}`}>
            {index + 1}
          </button>
        ))}
      </div>
