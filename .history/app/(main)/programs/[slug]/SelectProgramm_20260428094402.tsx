...
export default function ProgramSelect({ program, userId, initialTime }: ProgramSelectProps) {
  ...
  const defaultTime = initialTime && timePriceMap[initialTime] ? initialTime : times[0]
  ...
  return (
    <div className="mt-12 flex flex-col sm:flex-row items-center justify-between gap-6">
     {program.time.length > 3 ? <label className="flex">
        <p>Выберите часы:</p>
      ...
      <p className="!text-default !font-normal opacity-80">{program.time} академических {getHourWord(Number(program.time))}</p>
      } 
