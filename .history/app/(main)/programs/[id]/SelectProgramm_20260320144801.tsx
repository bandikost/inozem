...
export default function ProgramSelect({ program, userId }) {
  const times = program.time.split(",")       
  const prices = program.price.split(",")  

  const timePriceMap: Record<string, string> = {};
  times.forEach((t: any, i: any) => timePriceMap[t] = prices[i]);
  ...
  return (
    <div className="mt-6 flex flex-col gap-4">
      <label>
        Выберите часы:
        <select value={selectedTime} onChange={handleTimeChange} className="ml-2">
          {times.map(t => (
