...
export default function ProgramSelect({ program, userId }: ProgramSelectProps) {
  ...
  const handleTimeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedPrice(timePriceMap[value] ?? "0")
  };
  ...
  return (
    ...
    <div className="flex items-center gap-4 mt-4">
      <p className="!text-lg">Цена: {selectedPrice}₽</p>

      <PayButton price={selectedPrice} programId={program.id} userId={userId} name={program.name} time={program.time} />
    </div>
    </div>
  );
}