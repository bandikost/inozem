...
async function getProfile(token: string): Promise<ProgramsPageProps> {
...
export default async function Page({ params }: ProgramsPageProps) {
  ...
  return (
    <section className="prose mx-auto p-4 mt-27">
      ...
      {/* Здесь корректно рендерим ProgramSelect */}
      {hasAccess && <ProgramSelect program={program} userId={user.id} />}
    </section>
  );
}