...
export default async function Page({ params }: PageProps) {
  ...
  parse(activity.description, {
    replace: domNode => {
      ...
      <div className="mt-6">
        <button className="text-xl text-prpl mb-2">{activity.conditions ? "Условия участия" : ""}</button>
      </div>
      ...
      <div className="mt-6">
        <button className="text-xl text-prpl mb-2">{activity.purpose ? "Цель семинара" : ""}</button>
      </div>
      </div>
    </section>
  )
