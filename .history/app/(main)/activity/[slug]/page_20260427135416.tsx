...
export default async function Page({ params }: PageProps) {
  ...
  parse(activity.description, {
    replace: domNode => {
  ...
  }) : <h2 className="!text-xl text-center opacity-50">На данный момент информация о мероприятии отсутствует.</h2>}
      ...
      <div className="mt-6">
        ...
        {!activity.paylink ? (
          <div className="grid items-center gap-4 mt-10 px-2">
            <Link className="button-more-bulge !py-3 text-lg" href={`/activity-form?title=${encodeURIComponent(activity.title)}`}> Подать заявку</Link>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4 mt-10 px-2">
            <Link className="button-more-bulge !py-3 text-lg" href={`/activity-form?title=${encodeURIComponent(activity.title)}`}> Подать заявку</Link>
             <Link className="button-more !py-3 text-lg" href="">Оплатить участие</Link>
          </div>
        )}
