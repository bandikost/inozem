...
function getActivityDate(act: { dates: string; year?: number | null }) {
  const date = parseDateActivity(act.dates)
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {upcomingActivities.map((act) => (
              <div
                key={act.id}
                className="group rounded-xl border border-zinc-200 bg-white p-1 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl"
              >
                <div className="p-6">
                  <div className="mb-4 text-md font-medium text-zinc-400">
                    <span
                  ...
                  <h3 className="text-2xl text-prpl leading-tight">
                    {act.name}
                  </h3>

                  <Link
                    href={`/activity/${act.slug}`}
                    className="mt-5 inline-flex items-center gap-2 text-prpl hover:gap-3 transition-all hover:underline"
                  >
                    Подробнее →
                  </Link>
                </div>
