        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {upcomingActivities.map(act => (
              <div key={act.id} className="group rounded-xl border border-zinc-200 bg-white p-1 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
                <div className="flex gap-4">
                  
                  <div className="flex min-w-[120px] flex-col items-center justify-center rounded-lg bg-green text-white p-3">
                    <span className="text-md opacity-80">Дата</span>
                    <div className="text-center text-sm font-medium" dangerouslySetInnerHTML={{ __html: act.dates }} />
                  </div>

                  <div className="flex flex-col justify-between p-3">
                    <h3 className="text-prpl !text-xl font-semibold leading-snug">
                      {act.name}
                    </h3>

                    <Link href={`/activity/${act.slug}`} className="mt-4 text-prpl font-medium hover:underline">
                      Подробнее →
                    </Link>
                  </div>
