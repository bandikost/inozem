...
export default async function Page() {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {upcomingActivities.map(act => (
              <div key={act.id} className="group rounded-xl border border-zinc-200 bg-white p-1 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
                <div className="group overflow-hidden rounded-2xl border border-zinc-200 bg-white">
  <div className="h-2 bg-gradient-to-r from-prpl to-fuchsia-400" />
  ...
  <div className="p-6">
    ...
    <Link
      ...
      className="mt-5 inline-flex items-center gap-2 text-prpl hover:gap-3 transition-all"
    >
      Подробнее →
    </Link>
  </div>
