...
export default async function Page() {
  ...
  <div className="absolute left-3 top-0 bottom-0 w-[2px] bg-purple-200" />

  {upcomingActivities.map(act => (
    <div key={act.id} className="relative mb-8">
      <div className="absolute left-[-18px] top-2 h-4 w-4 rounded-full bg-prpl" />
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <Link
          ...
          className="mt-3 inline-block text-prpl hover:underline"
        >
          Подробнее →
        </Link>
      </div>
