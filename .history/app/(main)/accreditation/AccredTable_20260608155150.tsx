...
export default function AccredTable({ schedule }: Props) {
  ...
  return (
    <div className="my-10 flex flex-col justify-center items-center">
      ...
      {specialization && (
        <table className="w-full mt-6 border-collapse border border-gray-300 flex flex-col">
          <thead>
          ...
          <tbody>
            {filteredSchedule.map((item) => (
              <tr key={item.id}>
                <td className="border border-gray-300 px-4 py-3 text-left text-xl text-default">
                  {item.stage}
                </td>
