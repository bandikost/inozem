  ...
  return null
    ...
    return (
        <div className="flex flex-col sm:flex-row mt-8">
          ...
          {items.map(item => (
            <button key={item.id} onClick={() => setActiveId(item.id)} className={`text-lg px-2 py-2 button-more ${activeId === item.id ? "button-active" : ""}`}>
              {item.title}
            </button>
          ))}
            </div> 
               ...
               {activeItem?.links?.length > 0 && (
                  <ul className="flex gap-2 flex-col mt-5">
                    {activeItem.links.map((link, index) => (
                        ...
                        {link.url ? (
                            <a href={link.url} target="_blank" className="text-[#7C109A] underline hover:opacity-70 cursor-pointer flex flex-col items-center text-left">
                                <p className="flex items-center text-lg">{link.name}<MoveRight className="ml-1 min-w-[5px] max-w-[18px] w-full" /></p>
