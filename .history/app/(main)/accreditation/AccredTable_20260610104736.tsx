    ...
    <div className="divide-y divide-gray-100">
      {filteredSchedule.map((item) => (
        ...
        >
          ...
          <div className="col-span-4 text-center text-gray-600 border-r border-gray-300 px-4 text-left text-lg">
            Месяц: <span className="!text-teal-700">{item.month}</span> <br />
            День: <span className="!text-teal-700">{item.day}</span> <br /> 
            Время: <span className="!text-teal-700">{item.year}</span>
          </div>

          <div className="col-span-4 text-right text-blue-600 pl-4  text-lg">
            {item.time}
          </div>
        </div>
      ))}
