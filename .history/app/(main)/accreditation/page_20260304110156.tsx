    ...
    <section className="flex flex-col">
      <h1 className="text-[#A358B8] font-semibold mt-27 text-3xl text-center">Первичная специализированная аккредитация</h1>
        ...
        <div className="flex justify-between">
             ...
             <div className="flex flex-col mt-8 mr-4 gap-2">
          {items.map(item => (
            <button
              key={item.id}
              onClick={() => setActiveId(item.id)}
              className={`px-2 py-2 button-more ${activeId === item.id ? "bg-[#00C7B2]" : "bg-green  hover:opacity-70"}`}>
              {item.title}
            </button>
          ))}
        </div> 
        ...
        <div className="w-full border-2 border-dotted border-zinc-300 mt-8 rounded shadow-2xl bg-white px-6 py-3">
            <h3 className="text-xl text-center text-black mt-3">ГРАФИК ПЕРВИЧНОЙ СПЕЦИАЛИЗИРОВАННОЙ АККРЕДИТАЦИИ</h3>
