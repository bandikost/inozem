      ...
      <div className="border border-gray-300 shadow-2xl rounded-md p-4 w-[460px]">
        ...
        {feedback.map(feed => (
          <div key={feed.id} className="border border-gray-300 shadow rounded-md p-4 mt-5">
            ...
            <div className="flex">{new Date(feed.created_at).toLocaleDateString("ru-RU", {
    day: "2-digit",
    month: "long",
    year: "numeric",
  })}
  {" "}
  {new Date(feed.created_at).toLocaleTimeString("ru-RU", {
    hour: "2-digit",
    minute: "2-digit"
  })}<p className="mt-4 flex items-center">{feed.rate} <Star className="-mt-1"  fill="yellow" size={16} /></p></div>
          </div>
        ))}
      </div>
