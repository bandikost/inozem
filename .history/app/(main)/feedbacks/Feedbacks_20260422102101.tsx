  ...
  const renderCard = (feed: Feedback) => (
    ...
    <div key={feed.id} className="border border-gray-300 shadow rounded-md p-4 my-5 grid h-full min-h-[290px] shadow-xl">
      ...
      <div className="grid grid-cols-2 w-full items-center mt-6">
        ...
        {visibleForm && (
          <form onSubmit={(e) => handlePostAnswer(e, feed.id)} className="mt-4 grid grid-cols-2 gap-4 items-center">
            <input placeholder="Ответ от академии" value={answer} onChange={(e) => setAnswer(e.target.value)} maxLength={200} className="mt-4 border border-gray-400 text-lg rounded-md p-2"/>
            <button className="button-more" type="submit">Отправить ответ</button>
          </form>
        )}
    </>
  )
