      ...
      <div className="border border-gray-300 shadow-2xl rounded-md p-4 w-[460px]">
        ...
        {feedback.map(feed => (
          <div key={feed.id} className="border border-gray-300 shadow rounded-md p-4 mt-5">
            ...
            <p className="mt-2 ьи-4">{feed.user_text}</p>
             <hr className="border border-gray-300" />
            <div className="grid grid-cols-2 w-full">
              <p className="text-left">{new Date(feed.created_at).toLocaleDateString("ru-RU", {day: "2-digit", month: "long", year: "numeric"})}</p>
              <p className="flex justify-end items-center">{feed.rate} <Star className="-mt-1" fill="yellow" size={16} /></p>
            </div>
          </div>
        ))}
      </div>
