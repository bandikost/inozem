...
import { CircleUserRound, GraduationCap, LogOutIcon } from "lucide-react"
    ...
    return (
      ...
      <div className="grid grid-cols-1 tablet:grid-cols-2 items-start gap-8 mt-8">
        ...
        <div className="border border-gray-300 mt-8 rounded shadow-2xl bg-white pt-3 h-full flex flex-col">
          <h3 className="text-prpl font-semibold !text-2xl p-4 flex gap-2"><GraduationCap size={30} strokeWidth={1.5} /> Программы обучения</h3>
              ...
              {programs.map(p => {
                ...
                return (
                  <Link key={p.id} href={`/programs/${p.slug}`} className="hover:bg-[#be71cc] hover:!text-white transition-colors">
                    <li className="px-4 border border-gray-300 py-8 rounded-md">
                      <p className="font-semibold !text-xl !text-prpl">{p.name}</p>
                      <p className="!text-sm">Доступ открыт до: {endDate.toLocaleDateString()}</p>
                    </li>
                  </Link>
                )
              })}
