...
export default async function ProfilePage() {
  ...
  const getDaysLeft = (createdAt: string, time: number) => {
  const start = new Date(createdAt)
  const end = new Date(start)

  if (time < 71) end.setMonth(end.getMonth() + 1)       
  else if (time < 143) end.setMonth(end.getMonth() + 2) 
  else if (time < 287) end.setMonth(end.getMonth() + 3) 
  else if (time < 500) end.setFullYear(end.getFullYear() + 1) 
    ...
    <section className="flex flex-col justify-center items-center px-4">
      ...
      <div className="grid grid-cols-1 tablet:grid-cols-2 gap-8 mt-8">
        ...
        <div className="border border-gray-300 mt-8 rounded shadow-2xl bg-white pt-3">
          ...
          {programs.map(p => {
            const daysLeft = getDaysLeft(p.created_at, p.time)

            const endDate = new Date(p.created_at)
            if (p.time < 71) endDate.setMonth(endDate.getMonth() + 1)
            else if (p.time < 143) endDate.setMonth(endDate.getMonth() + 2)
            else if (p.time < 287) endDate.setMonth(endDate.getMonth() + 3)
            else if (p.time < 500) endDate.setFullYear(endDate.getFullYear() + 1)
