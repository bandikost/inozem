...
import { CalendarDays, ChevronRight, CircleUserRound, GraduationCap, LogOutIcon, Star } from "lucide-react"
...
export default function ProfileClient({programs, user} : Props) {
    ...
    return (
        ...
        <div className="border border-gray-300 mt-8 rounded shadow-2xl bg-white pt-3 flex flex-col">
          ...
          <ul className="flex flex-col p-6 gap-4">
              {visiblePrograms.map(p => {
                ...
                return (
                  <Link key={p.id} href={`/programs/${p.slug}`} className="hover:bg-[#be71cc] hover:!text-white group transition-colors">
          ...
          {programs.length > 2 && (
            <div className="px-6 pb-6">
              <button onClick={() => setShowAll(prev => !prev)}
                className="w-full border border-gray-300 rounded-md py-4 text-lg hover:bg-prpl hover:text-white transition-colors button-more">
      ...
      <div className="border border-gray-300 mt-8 rounded shadow-2xl bg-white pt-3 flex flex-col h-[570px]">
      ...
      <div className="gap-6 pt-3 flex items-center justify-between mt-20">
