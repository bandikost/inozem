      ...
      <div className="border border-gray-300 mt-8 rounded shadow-2xl bg-white px-6 py-3">
        <div className="grid grid-cols-2 w-full items-center">
            ...
            <div className="flex flex-col items-start" >
                {user.photo_url && <img src={user.photo_url}   width={100} height={100} className="w-[120px] h-[120px] rounded-full object-cover border-2 border-prpl" alt="Profile"/>}
                 {user.isTeacher ? (
            ...
            <ul className="text-default text-lg py-4 break-words">   
              <li><strong>Фамилия:</strong><br/><span className="relative -top-1.5">{user.last_name}</span></li>
              <li><strong>Имя:</strong><br/><span className="relative -top-1.5">{user.name}</span></li>
              <li><strong>Отчество:</strong><br/><span className="relative -top-1.5">{user.patronymic ?? ""}</span></li>
            </ul>
            <LogoutButton />
        </div>
        </div>
        ...
        <div className="border border-gray-300 mt-8 rounded shadow-2xl bg-white px-6 py-3">    
            {user.isTeacher ? (
              <h3 className="text-prpl font-semibold text-2xl p-4">Расписание</h3>
            ) : (
