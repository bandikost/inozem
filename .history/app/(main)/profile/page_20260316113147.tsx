  ...
  return (
    <section className="flex flex-col justify-center items-center px-4">
      ...
      <div className="grid grid-cols-1 gap-8 mt-8">
        ...
        <div className="border border-gray-300 mt-8 rounded shadow-2xl bg-white px-6 py-3">
          ...
          <ul className="flex flex-col gap-4 px-3 pb-4">
            {programs.map(p => (
              <Link key={p.id} href={`/programs/${p.id}`} className="!text-blue">
                <li className="p-4 rounded-lg shadow-md border border-gray-300 bg-card">
                  {p.name}  
                </li>
              </Link>
            ))}
      ...
      {user.isTeacher ? ( 
        ...
        <div className="flex flex-col p-3 min-w-0 border border-gray-300 mt-8 rounded shadow-2xl bg-white"> 
