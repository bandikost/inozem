...
export default function ProfileClient({programs, user} : Props) {
    ...
    return (
              ...
              {visiblePrograms.map(p => {
                ...
                return (
                  <Link key={p.id} href={`/programs/${p.slug}`} className="hover:bg-[#be71cc] hover:!text-white group transition-colors">
                    <div className="px-4 border border-gray-300 py-8 rounded-md flex flex-col items-center">
      ...
      <div className="pt-3 grid gap-6 grid-cols-1 tablet:grid-cols-2 items-center my-10">
        <LogoutButton /> 
        <Link className="flex p-8 justify-center text-2xl rounded-md items-center gap-1 hover:opacity-70 shadow-2xl bg-prpl !text-white" href="/dashboard/">
