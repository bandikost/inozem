                  ...
                  <Link key={p.id} href={`/programs/${p.slug}`} className="hover:bg-[#be71cc] hover:!text-white transition-colors">
                    <div className="px-4 border border-gray-300 py-8 rounded-md">
                      <div className="flex items-center">
                        <p className="font-semibold !text-xl !text-prpl text-lg">{p.name}</p>
                        <ChevronRight size={50} strokeWidth={1} />
                      </div>
                      <p className="!text-md !font-base flex items-center gap-2 mt-4"><CalendarDays size={18} strokeWidth={1.5} /> Доступ открыт до: {endDate.toLocaleDateString()}</p>
                    </div>
                  </Link>
                )
              })}
