  ...
  >
    {showPassword ? <EyeOff className="cursor-pointer hover:text-zinc-900" size={18} /> : <Eye className="cursor-pointer hover:text-zinc-900"  size={18} />}
        ...
        <div className="flex items-center">
          <input type="checkbox" className="cursor-pointer scale-125 border border-zinc-700" checked={checked} onChange={(e) => setChecked(e.target.checked) } />
          <p className="text-base sm:text-sm text-zinc-700 ml-0 sm:ml-2">Согласие на <Link target="_blank" href={"/files/personalize/personal-data.pdf"} className="text-blue hover:text-blue-700">обработку персональных данных</Link></p>
        </div>        
        

        <button type="submit"  disabled={loading || !checked} className="flex items-center px-4 py-2 bg-purple-600 text-white text-center rounded flex cursor-pointer hover:bg-prpl disabled:opacity-50 disabled:cursor-not-allowed">
          {loading ? "Регистрация..." : "Регистрация"}
        </button>
      </form>
        <p className="ml-4 mt-4 text-zinc-700">У вас уже есть личный аккаунт? <Link className="text-blue hover:text-blue-700" href={"/login"}>Авторизция</Link></p>
