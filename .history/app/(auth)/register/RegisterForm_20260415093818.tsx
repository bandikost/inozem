...
useEffect(() => {
  ...
  return (
    <div className="max-w-[400px] mx-auto mb-40 tablet:mb-28">
      <h1 className="text-prpl font-semibold text-3xl text-center mt-44 tablet:mt-27">Регистрация</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-5 border border-zinc-400 rounded-xl p-4 shadow-2xl">
        <input className="border border-zinc-400 p-1.5 rounded mt-4 text-zinc-700" name="name" placeholder="Имя" value={form.name} onChange={handleChange} required maxLength={30} />
        <input className="border border-zinc-400 p-1.5 rounded text-zinc-700" name="last_name" placeholder="Фамилия" value={form.last_name} onChange={handleChange} required maxLength={30} />
        <input className="border border-zinc-400 p-1.5 rounded text-zinc-700" name="patronymic" placeholder="Отчество" value={form.patronymic} onChange={handleChange} maxLength={30} />
        <input className="border border-zinc-400 p-1.5 rounded text-zinc-700" name="email" type="email" placeholder="Почта" value={form.email} onChange={handleChange} required maxLength={40} />

    <PhoneInput country={"ru"} value={form.phone} onChange={(phone) =>
        ...
        <select name="education_level" value={form.education_level} onChange={handleChange} required className="border border-zinc-400 p-2 w-full rounded text-zinc-700">
            ...
            <option value="без образования">Без мед.образования</option>
              ...
              {form.education_level !== "без образования" && (
                    ...
                    required={form.education_level !== "без образования"}  className="border border-zinc-400 p-2 w-full rounded text-zinc-700">

                    <option value="">-- выберите специальность --</option>

                    {(form.education_level === "Высшее" ? HIGHER_SPECIALTIES : SECONDARY_SPECIALTIES
                    ).map((spec) => (
...
<div id="yandex-captcha" className="mt-2" />
        ...
        <p className="ml-4 mt-4 text-zinc-700">У вас уже есть личный аккаунт? <Link className="text-blue hover:text-blue-700" href={"/login"}>Авторизация</Link></p>
        <p className="ml-4 mt-2 text-zinc-700 mb-10">Вернуться на <Link className="text-blue hover:text-blue-700" href={"/"}>главную</Link> </p>
