  ...
  return (
    <div className="mt-2 px-4 grid gap-2">
      <button
        ...
        className=" text-prpl text-2xl cursor-pointer hover:underline "
      >
        {title} ({open ? "Свернуть" : "Развернуть"})
      </button>
      {open && <div className="px-6 mt-2 text-default grid gap-2 mt-4 text-center">{children}</div>}
    </div>
  )
}
        ...
        <div className="flex flex-col mt-8">
          ...
          <div className="border-2 border-dotted border-zinc-300 pb-4 mt-8 rounded shadow-2xl bg-white px-6 py-3 grid gap-4">
            <ToggleBlock title="Бюджетные сметы образовательной организации:">
             ...
             <Link className="mt-20" href={""}>Отчет о результатах самообследования за 2015 год</Link>
          </div>
        </div>
      </div>
