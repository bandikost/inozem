...
export async function putPrograms(req: NextRequest) {
  const body = await req.json();
  const { name, time, dates, education, specialization, isFavorite, description, price } = body;

  const [result] = await db.query(
    `INSERT INTO programms (name, time, dates, education, specialization, isFavorite, description, price)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
