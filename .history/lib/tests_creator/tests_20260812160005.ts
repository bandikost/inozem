...
import { TestsCreatedContent } from "@/app/interface/tests_content";
...
export async function getTestsCreatedInfoContent(): Promise<TestsCreatedContent[]> {
  const [rows] = await db.query<(TestsCreatedContent & RowDataPacket)[]>(
    `
      SELECT 
        id,
        test_id,
