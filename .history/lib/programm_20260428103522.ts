...
export async function getPrograms(): Promise<ProgramRow[]> {
  ...
  const [rows] = await db.query<ProgramRow[]>(
    'SELECT id, name, slug, time, dates, education, specialization, isFavorite, bannerName, description, price FROM programms'
  )
    ...
    `SELECT up.id
     ...
     WHERE up.user_id = ?
       ...
       AND (
         ...
         (p.time >= 287 AND p.time < 600 AND up.created_at >= NOW() - INTERVAL 12 MONTH)
       )
     LIMIT 1`,
    [userId, programId]
  )
