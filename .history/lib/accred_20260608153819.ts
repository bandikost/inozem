...
export async function getAccredShedule(): Promise<Accred[]> {
    const [rows] = await db.query<Accred[] & RowDataPacket[]>(`
        select id, year, month, education, specialization, stage, name, link, created_at from accreditation_schedule`)
