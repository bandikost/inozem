    ...
    return (
        <section className="w-full flex flex-col gap-4 mt-20 px-4">
            ...
            <div className="flex flex-col items-center justify-center">
                        ...
                        {favoritePrograms.map((p) => (
                            <div key={p.id} className="flex flex-col justify-between gap-8 bg-white p-4 rounded-lg shadow-md border border-gray-300">
                                ...
                                <div className="grid grid-cols-2 items-center">
                                    <p className="flex mt-2 items-center"><Clock9 className="w-4 h-4 mr-1 -mt-0.5" />{p.time} академ. {getHourWord(Number(p.time))}</p>
                ...
                <Link href={"/learning"} className="!text-lg  mt-4 hover:opacity-80 hover:underline">Перейти в общий раздел обучения</Link>
            </div>
