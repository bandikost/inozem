                    ...
                    <div className="grid grid-cols-1 tablet:grid-cols-3 gap-4 mt-8">
                        {favoritePrograms.map((p) => (
                            <div key={p.id} className="flex flex-col justify-between gap-8 p-4 rounded-lg shadow-md border border-gray-300 bg-card">
                                    <h3 className="md:text-left text-center !font-semibold text-prpl">{p.name.length > 58 ? p.name.slice(0, 58) + "..." : p.name}</h3>
                                ...
                                <div className="grid grid-cols-2 items-center">
                                    <p className="flex mt-2 items-center"><Clock9 className="w-4 h-4 mr-1 " />{p.time.length > 3 ? `от ${p.time.slice(0, 2)}` : p.time} академ. {getHourWord(Number(p.time))}</p>
                                    <Link href={`/programs/${p.slug}`} className="button-more">
                                        Подробнее
                                    </Link>
                                </div>
                            </div>
