...
export default async function Programms() {
  ...
  ...p,
  time: p.time ? JSON.parse(p.time) : [],
}))

    const favoritePrograms = parsedPrograms.filter(p => p.isFavorite)
                        ...
                        {program.bannerName && ( 
                            ...
                            <div className="flex items-center justify-between mt-6">
                                <div className="flex flex-col"> 
                                    {program.time?.length >= 2 ? (
                                    <p className="!font-normal text-zinc-800 flex items-center !text-base">
                                        <Clock9 className="w-4 h-4 mr-1 mt-0.1" />
                                        от {program.time[0]} акад. {getHourWord(program.time[0])}
                                    </p>
                                    ) : (
                                        <p className="!font-normal text-zinc-800 flex items-center !text-base">
                                        <Clock9 className="w-4 h-4 mr-1 mt-0.1" />
                                        {program.time[0]} акад. {getHourWord(program.time[0])}
                                    </p>
                                    )}
                                </div>
