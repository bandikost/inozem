    ...
    return (
        <section className="grid grid-cols-2 justify-center gap-8 mt-10">
            <div className="border-2 border-dotted border-zinc-300 rounded shadow-2xl">
                ...
                <ul>
                {teachers.map(teach => (
                    <li key={teach.id} className="flex items-center px-6 py-2 gap-4">
                    {teach.photo_url ? (
                        <img 
                        src={teach.photo_url} 
                        alt={`${teach.first_name} ${teach.last_name}`} 
                        className="w-12 h-12 rounded-full object-cover border" 
                        />
                    ) : (
                        <div className="border rounded-full w-12 h-12 bg-black"></div>
                    )}

                    <p className="ml-2">{teach.first_name} {teach.last_name}</p>
                    </li>
                ))}
                </ul>
