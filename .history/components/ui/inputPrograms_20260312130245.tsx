...
export default function InputPrograms({ programs }: { programs: any[] }) {
    ...
    return (
        ...
        <div className="border border-zinc-200 rounded-xl p-6 bg-white shadow-xl transition">
            <h3 className="text-prpl mb-4 !font-normal">Введите название программы</h3>
            <div className="flex items-center">
                <input value={inputValue} onChange={(e) => setInputValue(e.target.value) } className="border border-gray-300 rounded p-2" />
                <button className="button-more" onClick={() => handleFindProgramm()}>Поиск</button>
            </div>
            
        </div>
         {findTotal.length === 0 && (
                <p className="mt-10 text-zinc-700 !text-lg flex items-center justify-center">Программ с названием <span className="text-blue">"{inputValue}"</span> не найдено.</p>
            )}
