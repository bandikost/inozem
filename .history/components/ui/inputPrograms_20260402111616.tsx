...
export default function InputPrograms({ programs }: { programs: ProgramRow[] }) {
    ...
    <section className="flex flex-col items-center computer:items-stretch justify-center">
        <h2 className="my-12 text-prpl text-center sm:text-left !text-3xl">Поиск программ по критериям</h2>
    <div className=" border border-zinc-200 rounded-xl p-3 tablet:p-6 bg-white shadow-xl transition ">
        ...
        <div className="flex flex-col computer:flex-row justify-between gap-4 items-center mt-2">
            <div className="flex flex-col items-start gap-3 -ml-7 computer:ml-0">
                ...
                <input value={inputValue} onChange={(e) => setInputValue(e.target.value) } className="border border-gray-300 rounded p-2 w-[240px] xs:w-[300px]" placeholder="Введите название..." />
            </div>
            ...
            <div className="flex flex-col items-start  gap-3 mt-10 tablet:mt-0">
                ...
                <div className="flex items-start gap-3">
                    <select value={education} onChange={(e) => setEducation(e.target.value)} name="education_level"  
                        className="border border-zinc-400 p-2 rounded text-zinc-700 w-[240px] xs:w-[300px]">
        ...
        {filteredPrograms.length === 0 ? (
         ...
         ) : ( <h2 className="mt-12 text-prpl text-center sm:text-left !text-3xl">Все программы</h2> )}
