               ...
               <hr className="border border-1 border-gray-300 mt-3" />
                  <ul className="grid grid-cols-2">
                    {activeItem.links.map((link, index)=> (
                      <li key={index}>
                          <a href={link.url} target="_blank" className="text-base sm:!text-lg text-[#7C109A] hover:bg-blue hover:!text-white cursor-pointer flex flex-col items-center border border-gray-300 p-3">
                            {link.name}
                          </a>
                        </li>
                    ))}
