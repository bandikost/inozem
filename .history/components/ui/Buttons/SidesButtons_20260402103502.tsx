               ...
               <h2 className="text-xl text-green text-center mt-2">{activeItem?.title}</h2>
                  <ul className="grid grid-cols-2 gap-1  mt-5">
                    {activeItem.links.map((link, index)=> (
                      <li key={index}>
                          <a href={link.url} target="_blank" className="text-[#7C109A] hover:bg-blue hover:!text-white cursor-pointer flex flex-col items-center border border-gray-300 rounded p-3">
                            {link.name}
                          </a>
                        </li>
                    ))}
