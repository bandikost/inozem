    ...
    return (
        ...
        <div className="flex flex-col mr-4 gap-2">
          {learning.map(item => (
            ...
            <div className="w-full sm:w-3/4 border border-gray-300 rounded shadow-xl bg-white py-3 mt-8 sm:mt-0">
               <h2 className="text-xl text-green text-center mt-2">{activeItem?.title}</h2>
                  <ul className="grid grid-cols-2 gap-2  mt-5">
                    {activeItem.links.map((link, index)=> (
                      <li key={index}>
                          <a href={link.url} target="_blank" className="text-[#7C109A] underline hover:opacity-70 cursor-pointer flex flex-col items-center border border-gray-300 rounded ">
