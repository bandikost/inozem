        ...
        <div className="flex flex-col mr-4 gap-2">
          {learning.map(item => (
            <button
              ...
              className={`px-0 py-2 button-more text-lg  ${activeId === item.id ? "button-active" : ""}`}>
              {item.title}
            </button>
          ))}
        </div> 
               ...
               <hr className="border border-1 border-gray-300 mt-3" />
                  <ul className="grid grid-cols-2">
                    {activeItem.links.map((link, index)=> (
                      <li key={index}>
                          <a href={link.url} target="_blank" className="!text-lg text-[#7C109A] hover:bg-blue hover:!text-white cursor-pointer flex flex-col items-center border border-gray-300 p-3">
                            {link.name}
                          </a>
                        </li>
                    ))}
