                ...
                {openMenu && (
                    <>
                        ...
                        <div className="flex flex-col items-center  h-[280px] w-[280px]">
                            ...
                            <hr className="border border-gray-300 w-full" />
                            <Link className="button-more mt-4" href={"/bid"} onClick={handleOpenMenu}>Перейти к подаче заявки</Link>
                        </div>
                    </>
                )}
            </div>
            {!openMenu && (
            <button className="button-more shadow transform transition-all duration-300 hover:scale-105 !text-lg flex gap-2" onClick={handleOpenMenu}>
                <MessageCircleQuestionMark />  Задать вопрос 
            </button>
            )}
        </section>
    )
