        ...
        <div className="flex flex-col mr-4 gap-2">
          {learning.map(item => (
            ...
            <div className="w-full sm:w-3/4 border border-gray-300 rounded shadow-xl bg-white pt-3 mt-8 sm:mt-0">
               ...
               <hr className="border border-1 border-gray-300 mt-2" />
                  <ul className="grid grid-cols-2 gap-1  mt-5">
                    {activeItem.links.map((link, index)=> (
                      <li key={index}>
                          <a href={link.url} target="_blank" className="!text-lg text-[#7C109A] hover:bg-blue hover:!text-white cursor-pointer flex flex-col items-center border border-gray-300 rounded p-3">
                            {link.name}
                          </a>
                        </li>
                    ))}
