    ...
    return (
        <section className="fixed -right-0.5 -bottom-1 z-10">
            ...
            <div className={`bg-white border border-gray-300 rounded-md shadow-2xl relative transform transition-all duration-300 ease-in-out
                    ${openMenu ? "opacity-100 scale-100 m-0 sm:mr-5 sm:mb-5" : "opacity-0 scale-75 pointer-events-none"}`}>
                {openMenu && (
                    <>
                        <button className="absolute right-2 top-10 sm:-right-2 sm:-top-2 hover:opacity-80 text-white" onClick={handleOpenMenu}>
                        ...
                        <div className="flex flex-col items-center w-screen h-screen sm:h-[280px] sm:w-[280px]">
                            <p className="mt-1 py-3 px-4 !text-base">
                                Добрый день! Часто задаваемые вопросы находятся  
                                <Link className="hover:underline" href={"/contacts"} onClick={handleOpenMenu}> в разделе</Link>
                            </p>
            ...
            {!openMenu && (
            <button className="!text-white button-more shadow transform transition-all duration-300 hover:scale-105 !text-lg flex gap-2" onClick={handleOpenMenu}>
                <MessageCircleQuestionMark className="text-white" />  Задать вопрос 
            </button>
            )}
        </section>
    )
