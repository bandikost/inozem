...
export default function InputPrograms({ programs }: { programs: ProgramRow[] }) {
  ...
  filtered = filtered.filter((p) => {
    const specialties = p.specialization
      ?.split(",")
    ...
    // Подходит любая высшая специальность
    if (
      p.education?.toLowerCase() === "высшее" &&
      specialties.includes("*all_higher*")
    ) {
      return true;
    }

    // Подходит любая средняя специальность
    if (
      p.education?.toLowerCase() === "среднее" &&
