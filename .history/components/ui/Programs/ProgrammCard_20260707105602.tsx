              ...
              <div className="rounded-xl bg-zinc-50 p-4">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    {dates.slice(0, 2).map(date => (
                        <p className="!font-normal" key={date}>{date}</p>
                    ))}
                ...
                <button className="mt-4 text-md text-center !font-normal cursor-pointer border border-gray-200 px-2 py-1 rounded-xl hover:bg-prpl hover:text-white" onClick={() => toggleDates(program.id)}> {dates.length - 2 < 0 ? "" : `и еще ${dates.length - 2} ${filtredName} ${openedDates === program.id ? "Скрыть" : "Показать"}` }</button>
