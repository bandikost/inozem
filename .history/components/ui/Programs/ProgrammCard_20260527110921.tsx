    ...
    return (
         <div className="flex flex-col items-center gap-8 w-full">
                ...
                {filteredPrograms.slice(0, visibleItems).map(program => (
                    <div key={program.id} className="w-full max-w-[750px] border border-zinc-200 rounded-xl px-5 py-6 bg-white shadow-xl transition bg-card relative">
                    <h2 className="text-center !font-semibold custom-text">
                    {program.name.length > 58 ? program.name.slice(0, 58) + "..." : program.name}
                    <p className="text-black/60 !text-lg group-hover:text-white transition-colors">{program.time >= 432 && "(профессиональная переподготовка)"}</p>
                    </h2>
                    ...
                    <div className="grid gap-1 mt-4">
                        ...
                        <p className="!text-lg"><strong className="text-blue">Направления:</strong> {program.specialization}</p>
                    </div>
