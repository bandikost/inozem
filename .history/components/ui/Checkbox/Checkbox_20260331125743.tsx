// components/FormApplication.tsx
'use client'

import CheckBox152 from "@/components/ui/Checkbox/Checkbox"
import { useState } from "react"

export default function CheckBox({ user, programs }) {
    ...
    return (
        <form onSubmit={handleSubmit} className="border border-gray-300 py-4 px-6 rounded-md flex flex-col gap-4">
            ...
            <input required className="border py-1 px-2 border-gray-300 rounded-md w-[350px] text-default text-lg"
            ...
            <select required className="border py-1 px-2 border-gray-300 rounded-md max-w-[350px] text-lg">
                {programs.map((p, index) => (<option key={index}>{p.name}</option>))}
            </select>
            <CheckBox152 />
        </form>
    )
