...
export default function FormApplication({ user, programs }: any) {
    ...
    return (
        <form onSubmit={handleSubmit} className="border border-gray-300 py-4 px-6 rounded-md flex flex-col gap-4 w-[450px]">
            ...
            <select name="education_level" value={education_level} onChange={(e) => {
        setEducation_level(e.target.value)
        setSpecialization("") 
    }}required className="border border-zinc-400 p-2 w-full rounded text-zinc-700">
    ...
    <select
        ...
        required={education_level !== "без образования"}
