  ...
  return (
    <div className="max-w-4xl mx-auto my-10">
        ...
        <div className="grid grid-cols-2 gap-6">
             <select value={selected} onChange={(e) => setSelected(e.target.value)} className="border p-2 rounded mb-6 w-full">
        <option value="">Выберите специальность (Высшее)</option>

        {HIGHER_SPECIALTIES.map((s) => (
        <option key={`higher-${s}`} value={s} onChange={(s) => setEducation("Высшее")}>
            {s}
        </option>
        ))}
    ...
    <select value={selected} onChange={(e) => setSelected(e.target.value)} className="border p-2 rounded mb-6 w-full">
        ...
        <option key={`secondary-${s}`} value={s} onChange={(s) => setEducation("Среднее")}>
            {s}
        </option>
        ))}
      ...
      {!loading && selected && dates.length === 0 && (
        <p className="text-zinc-700 flex !font-semibold text-lg">
         <Hourglass className="animate-wait mr-2 text-green" />
        </p>
      )}

      {!loading && dates.length > 0 && (
        <div>
        <h2 className="text-xl text-zinc-700">{education}</h2>
