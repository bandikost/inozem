...
const handleSecondaryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  return (
    <div className="max-w-4xl mx-auto my-10">
        ...
        <option key={`higher-${s}`} value={education === "Высшее" ? selected : ""} onChange={handleHigherChange}>
            {s}
        </option>
        ))}
    ...
    <select value={selected} onChange={(e) => setSelected(e.target.value)} className="border p-2 rounded mb-6 w-full">
        ...
        <option key={`secondary-${s}`} value={education === "Среднее" ? selected : ""} onChange={handleSecondaryChange}>
            {s}
        </option>
        ))}
